...
# ══════════════════════════════════════════════════════════════════════════════
# 2. CONEXÃO SEGURA E RENOVAÇÃO TOTAL DO BANCO DE DADOS (V3)
# ══════════════════════════════════════════════════════════════════════════════
...
def reset_and_migrate_db():
    """Esta função GARANTE que o banco está na versão mais recente, apagando tudo se necessário."""
    with get_conn() as conn:
        with conn.cursor() as c:
            # 1. TENTA verificar se a estrutura está atualizada.
            try:
                c.execute("SELECT column_name FROM information_schema.columns WHERE table_name='diario_bordo' AND column_name='status'")
                # Se achou a coluna, ótimo. Não faz nada.
                if c.fetchone():
                    return 
            except Exception:
                conn.rollback()

            # 2. Se a coluna não existir (ou der erro), APLICA O DROP EM TUDO (FORÇADO)
            ...
            # 3. RECRIAR TUDO COM A ESTRUTURA V3
            queries = [
...
# Roda a verificação/limpeza ANTES DE QUALQUER TELA
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    with aba_s:
        ...
        if not v_disp or not c_atv: st.warning("Cadastre carros/motoristas.")
        else:
            with st.form("fs"):
                c1, c2 = st.columns(2)
    ...
    with a_c:
        ...
        if upc and st.button("Salvar Excel Pessoas"):
            ...
            for _, r in d.iterrows():
                ...
                execute_query("INSERT INTO condutores (nome,cnh,validade_cnh,cc_padrao) VALUES (%s,%s,%s,%s) ON CONFLICT(cnh) DO NOTHING", (r['nome'], str(r['cnh']), r['validade_cnh'], str(r['cc_padrao'])))
            st.success("Ok!"); st.rerun()
    ...
    with a_v:
        ...
        if upv and st.button("Salvar Excel Carros"):
            ...
            for _, r in d.iterrows():
                ...
                execute_query("INSERT INTO veiculos (placa,modelo,cc_atual,custo_fixo_mensal) VALUES (%s,%s,%s,%s) ON CONFLICT(placa) DO NOTHING", (str(r['placa']).upper(), r['modelo'], str(r['cc_atual']), r['custo_fixo_mensal']))
            st.success("Ok!"); st.rerun()
